...
public class STOMPMessagesHandler {
    ...
    public void handlePointEvent(Point pt,@DestinationVariable String numdibujo) throws Exception {
        System.out.println("Punto recibido en el Servidor!:"+pt);
        message.convertAndSend("/topic/newpoint."+numdibujo, pt);
        ArrayList<Point> points = getPointsByName(numdibujo);
        points.add(pt);
        if(points.size() >= 4){
            message.convertAndSend("/topic/newpolygon."+numdibujo, points);
        }
    }
